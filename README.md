...
- Three classes, `Circle.cs`, `Triangle.cs`, and `Rectangle.cs` were created and these classes inherits from `Shape.cs`.These three classes has properties that will help calculate area and perimeter depending on whether the user wants to `Draw()` the type of **Shape**: **Circle**, **Triangle**, or **Rectangle**.
...
![Console](/References/Capture.PNG)